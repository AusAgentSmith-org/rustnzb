...
pub async fn run(cfg: StressConfig) -> Result<()> {
    tracing::info!("============================================================");
    tracing::info!("  benchnzb v2: Stress Test (RustNZB only)");
    tracing::info!("============================================================");
    ...
    let rnzb = RustnzbClient::new(RUSTNZB_API);
    ...
    metrics_collector.resolve_container_id("rustnzb").await;

    let rnzb_container_id = docker::get_container_id(&docker_client, "rustnzb").await;

    // Start Docker stats collection
    let stats_handle = metrics_collector.start_collecting("rustnzb");
    ...
    if !rnzb_logs.is_empty() {
        let log_path = cfg.results_dir.join(format!("stress_rustnzb_{timestamp}.log"));
        std::fs::write(&log_path, &rnzb_logs)?;
...
async fn feeder_loop(
    ...
    loop {
        ...
        let queue_size = match rnzb.queue_size().await {
            Ok(n) => n,
            Err(e) => {
        ...
        if queue_size < target_depth {
            ...
            for _ in 0..to_add {
                ...
                match rnzb.add_nzb(&nzb_data, &filename).await {
                    Ok(()) => {
...
/// Collect metrics from RustNZB API at regular intervals.
async fn metrics_loop(state: Arc<SharedState>, rnzb: RustnzbClient, interval_secs: u64) {
    ...
    loop {
        ...
        // Get RustNZB status
        let (speed_bps, queue_size, active) = match rnzb.get_status().await {
            Ok(status) => (
                status.speed_bps,
                status.queue_size,
        ...
        if let Ok(history) = rnzb.history_count().await {
            let prev = state.nzbs_completed.load(Ordering::Relaxed);
            if history > prev {
                state.nzbs_completed.store(history, Ordering::Relaxed);
            }
        }
...
async fn cleanup_loop(
    ...
    loop {
        ...
        // Clean completed downloads from disk
        if let Some(cid) = docker::get_container_id(&docker, "rustnzb").await {
        ...
        // Clear rustnzb history to prevent unbounded growth
        let _ = rnzb.clear_history().await;
