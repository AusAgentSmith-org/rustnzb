...
async fn test_fetch_single_article_and_decode() {
    ...
    match &decode_result {
        ...
        Err(e) => {
            ...
            eprintln!("First 200 bytes of raw data: {:?}",
...
async fn test_fetch_multiple_articles_from_rar() {
    ...
    let rar_file = job.files.iter()
        .find(|f| f.filename.to_lowercase().contains(".r") && f.articles.len() > 5)
        .expect("No multi-segment RAR file found");

    eprintln!("Target RAR: {} ({} articles, {:.1} MB)",
        rar_file.filename, rar_file.articles.len(),
    ...
    for (i, article) in articles_to_fetch.iter().enumerate() {
        eprintln!("Fetching article {}/{}: segment {} (msg-id: {}...)",
            i + 1, articles_to_fetch.len(),
        ...
        match yenc::decode_yenc(&raw_data) {
            Ok(result) => {
                eprintln!("  Decoded: {} bytes, offset: {:?}, CRC: {:08X}",
                    result.data.len(), result.part_begin, result.crc32);
    ...
    eprintln!("\nSummary: fetched {} articles, decoded {} bytes total",
        articles_to_fetch.len(), total_decoded
    );
...
async fn test_article_not_found_handling() {
    ...
    let result = conn.fetch_article("nonexistent-fake-id-12345@nowhere.invalid").await;
    ...
    assert!(conn.is_connected(), "Connection should still be alive after 430");
    let _ = conn.quit().await;
