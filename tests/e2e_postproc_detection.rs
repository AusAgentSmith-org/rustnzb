...
use nzb_postproc::detect::{find_archives, find_cleanup_files, find_par2_files, ArchiveType};
use nzb_postproc::pipeline::{run_pipeline, PostProcConfig};
...
fn normal_subjects_par2_detected() {
    ...
    let filenames: Vec<String> = subjects.iter().map(|s| extract_filename_from_subject(s)).collect();
...
fn obfuscated_subject_quoted_hash() {
    ...
    assert_eq!(par2_files.len(), 0, "BUG: par2 detection fails for obfuscated names");
    assert_eq!(archives.len(), 0, "BUG: archive detection fails for obfuscated names");
}
...
fn obfuscated_subject_uuid_style() {
    ...
    assert_eq!(par2_files.len(), 0, "BUG: par2 detection fails for UUID-obfuscated names");
    assert_eq!(archives.len(), 0, "BUG: archive detection fails for UUID-obfuscated names");
}
...
async fn obfuscated_pipeline_skips_everything() {
    ...
    assert!(result.success, "Pipeline reports success even though nothing was processed");
...
fn deobfuscation_renames_files_correctly() {
    ...
    let par2_files = find_par2_files(dir.path());
    assert_eq!(par2_files.len(), 2, "Should find 2 par2 files after deobfuscation");
...
fn deobfuscation_skips_when_names_match() {
    ...
    let renames = simulate_deobfuscation(dir.path(), &nzb_filenames, &yenc_names);
    assert_eq!(renames.len(), 0, "No renames needed when names already match");
...
fn deobfuscation_handles_path_components_in_yenc_name() {
    ...
    let yenc_names: HashMap<String, String> = HashMap::from([
        ("f1".into(), "some/path/Movie.2024.par2".into()),
    ]);
...
fn extract_filename_various_subject_formats() {
    ...
    assert!(!name.ends_with(".par2") && !name.ends_with(".rar"),
...
fn old_style_rar_detection() {
    ...
    let archives = find_archives(dir.path());
    assert_eq!(archives.len(), 1, "Only first volume (.rar) should be returned");
    assert_eq!(archives[0].0, ArchiveType::Rar);
    assert!(
        archives[0].1.file_name().unwrap().to_str().unwrap().ends_with(".rar"),
