...
async fn test_upload_nzb_and_verify_queue() {
    ...
    eprintln!("SABnzbd fullstatus paused: {}", fullstatus["status"]["paused"]);
    ...
    assert!(sab_slots[0]["nzo_id"].as_str().unwrap().starts_with("SABnzbd_nzo_"));
    assert!(sab_slots[0]["filename"].is_string());
    assert!(sab_slots[0]["percentage"].is_string());
    eprintln!(
        "SABnzbd queue slot: {} ({}%)",
        sab_slots[0]["filename"],
        sab_slots[0]["percentage"]
    );
    ...
    assert_eq!(
        sab_history["history"]["slots"].as_array().unwrap().len(),
        0
    );
    ...
    let resp = client
        ...
        .await
        .unwrap();
    assert_eq!(resp.json::<serde_json::Value>().await.unwrap()["status"], true);
