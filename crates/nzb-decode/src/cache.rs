...
impl ArticleCache {
                ...
                if let Some(entry) = inner.entries.get_mut(&evict_key) {
                    if let EntryLocation::Memory(_) = &entry.location {
                        ...
                        if let EntryLocation::Memory(mem_data) = std::mem::replace(
                            ...
                            inner.memory_bytes =
                                inner.memory_bytes.saturating_sub(entry_size);
    ...
    pub fn clear_job(&self, job_id: &str) {
        ...
        debug!(job_id, removed = keys_to_remove.len(), "Cleared job from cache");
    }
...
mod tests {
    ...
    fn test_eviction_spills_to_disk() {
        ...
        let stats = cache.stats();
        assert!(stats.disk_spills >= 1, "Expected at least 1 disk spill, got {}", stats.disk_spills);
