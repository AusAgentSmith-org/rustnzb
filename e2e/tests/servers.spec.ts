...
async function apiListServers(
  token: string,
): Promise<Array<{ id: string; name: string; host: string }>> {
...
async function apiAddServer(
  ...
  if (!r.ok)
    throw new Error(`apiAddServer failed: ${r.status} ${await r.text()}`);
}

async function apiDeleteServerByName(
  token: string,
  name: string,
): Promise<void> {
  ...
  for (const target of targets) {
    ...
    if (!r.ok && r.status !== 404)
      throw new Error(`apiDeleteServer failed: ${r.status}`);
  }
}

async function navigateToServers(
  page: import("@playwright/test").Page,
): Promise<void> {
  await page.goto("/settings");
  // "News servers" is the default sidebar tab; ensure it is active
  await expect(
    page.getByRole("heading", { name: "News servers" }),
  ).toBeVisible();
}
...
test.describe("2. News Server Management", () => {
  ...
  test("2.1 add a news server", async ({ page }) => {
    ...
    const connectionsInput = page.locator(
      'label:has-text("Connections") + .inline input[type="number"]',
    );
    ...
    await expect(
      page.locator(".srv-row", { hasText: serverHost }),
    ).toBeVisible();

    // Snackbar confirms
    await expect(
      page.getByText("Server added", { exact: false }),
    ).toBeVisible();
  ...
  test("2.3 edit server connections count", async ({ page }) => {
    ...
    // Snackbar
    await expect(
      page.getByText("Server updated", { exact: false }),
    ).toBeVisible();

    // Restore to original value
    const testServerRowAgain = page.locator(".srv-row", {
      hasText: "Test Server",
    });
    await testServerRowAgain.getByRole("button", { name: "Edit" }).click();
    const connectionsInputAgain = page.locator(
      'label:has-text("Connections") + .inline input[type="number"]',
    );
  ...
  test("2.4 delete server with confirm dialog", async ({ page }) => {
    ...
    const token = readToken();
    await apiDeleteServerByName(token, "Temp Delete Server");
    await apiAddServer(token, "Temp Delete Server", "temp.delete.test.com");

    await navigateToServers(page);

    // Confirm the temp server appears
    const tempRow = page.locator(".srv-row", {
      hasText: "temp.delete.test.com",
    });
    await expect(tempRow).toBeVisible();

    // Confirm deletion through the shared in-theme dialog.
    await tempRow.getByRole("button", { name: "Remove" }).click();
    const dialog = page.getByRole("dialog");
    await expect(
      dialog.getByRole("heading", { name: "Remove this server?" }),
    ).toBeVisible();
    ...
    // Snackbar
    await expect(
      page.getByText("Server removed", { exact: false }),
    ).toBeVisible();
  });
  ...
  test("2.6 toggle server enabled state via row button", async ({ page }) => {
    ...
    try {
      ...
      await expect(
        page.getByText("Server disabled", { exact: false }),
      ).toBeVisible();
      ...
      await expect(
        page.getByText("Server enabled", { exact: false }),
      ).toBeVisible();
      await expect(row.getByText("● enabled")).toBeVisible();
      ...
      const servers = await apiListServers(token);
      const found = servers.find((s) => s.name === serverName) as
        | { enabled?: boolean }
        | undefined;
